...
class Jacobi {
    ...
    bool solve (int max_iter = 10000) {
    std::filesystem::create_directories(filename.parent_path());
    std::ofstream ofs(filename);
    std::unordered_set<size_t> SaveTimes;
    SaveTimes.insert(1000);
    SaveTimes.insert(2000);
    SaveTimes.insert(5000);
    SaveTimes.insert(10000);
    ...
    while (iter < max_iter) {
        ...
        if (SaveTimes.contains(iter))
          {
              std::cout<< "Iteratioin " << iter << ": Error = " << err << "Saveing to [" << filename.string() << std::endl;
    ...
    void Save(std::filesystem::path filename) {
        std::ofstream ofs(filename); // 出力ファイルを書き出し用に準備
        if (!ofs) {
            std::cerr << "Error opening file: " << filename << std::endl;
            return;
        }

        for (size_t i = 0; i < mySOL.size(); ++i) {
            double x = Xmin + (i + 1) * dx; // x座標を計算
            ofs << x << " " << mySOL[i] << std::endl; // x座標とy座標を出力
        }
